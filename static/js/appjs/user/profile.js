(function($) {
    ...
    function drawLineChart() {
        ...
        $.ajax({
            ...
            if (graph_data == 0) {
                $("#contest_graphs").html("<p>Graph data would be populated soon</p>")
                return;
            }
            ...
            function lineChartSelectHandler(e) {
                ...
                $.each(contestGraphData[dateStr], function(ind, val) {
                    if (val) {
                        tbodyHTML += "<tr><td>" + val[1]["name"] + "</td><td><img src='/stopstalk/static/images/" + urlToSite(val[1]["url"]) + "_small.png' style=\"height: 30px; width: 30px;\"></img></td><td><a class='btn-floating btn-small accent-4 green' href='" + val[1]["url"]+ "' target='_blank'><i class='fa fa-external-link-square fa-lg'></i></a></td><td>" + val[0] + "</td><td>" + val[1]["rank"] + "</td></tr>";
                    }
                });
